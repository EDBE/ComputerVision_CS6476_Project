...
% %initialize the accumulate value of cur_image
% image_total = 0;
...
for set = 1:length(sets)
    for category = 1:length(categories)
        ...
        for i = 1:length(cur_images)
            ...
            % concatenate grayscale images with themselves to make RGB
            % image
            cur_image = cat(3, cur_image, cur_image, cur_image);
            % normalization by subtracting the average image, using VGG-F
            % provided 'averageImage'
            cur_image = cur_image - single(averageImage);
