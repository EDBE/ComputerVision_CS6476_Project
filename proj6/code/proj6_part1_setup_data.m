...
image_size = [64 64];
%downsampling data for speed and because it hurts
...
total_image = zeros(image_size);
average_image = zeros(image_size);
for set = 1:length(sets)-1
    for category = 1:length(categories)
        ...
        if(set == 1)
            fprintf('Taking %d out of %d images in %s\n from the training set', num_train_per_category, length(cur_images), cur_path);
        ...
        for i = 1:length(cur_images)
            ...
            total_image = total_image + cur_image;
...
average_image = total_image / (image_counter-1);
...
for set = 1:length(sets)
    for category = 1:length(categories)
        ...
        for i = 1:length(cur_images)
            ...
            cur_image = cur_image - average_image;
