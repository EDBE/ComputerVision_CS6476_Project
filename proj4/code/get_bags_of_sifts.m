...
for i=1:N_images
    ...
    [indices, ~] = knnsearch(vocab, features', 'K', 1);
    
%?????????????????????????????????????????????
% This is the implementation of 'soft assignment'
%
...
%     for u=1:N_features
%         for v=1: 3
%             image_feats(i, indices(u,v)) = weights(u,v) + image_feats(i, indices(u,v));
%         end
%     end
%?????????????????????????????????????????????
    image_feats(i,:) = histc(indices, 1:vocab_size)';
end
% image_feats = bsxfun(@rdivide, image_feats, sum(image_feats,2));
end
